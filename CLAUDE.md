...
- **Boutons sidebar natifs (v2.2.6)** : les icônes tab-nav `#kt_aside_nav` changent l'onglet actif dans `#kt_aside_workspace`, mais depuis v1.9.58 ce workspace vit dans `#artis-menu-popup` — un `click` sur ces boutons sans handler artis-menu-open ne montrait rien. Fix : listener `click` sur `#kt_aside_nav` appelle `open()` (ajoute `html.artis-menu-open`).
- **Popup sélecteur thème (v2.2.6)** : le bouton `#artis-theme-toggle` n'est plus un toggle binaire — il ouvre `#artis-theme-popup` (body-level, `position:fixed`) avec 3 options Sombre/Clair/Limité. Se ferme sur click extérieur. Styles via `.artis-theme-option` / `.artis-theme-option--active`.
- **Popup aperçu CR Reformuler (v2.2.6)** : `showReformPreview(html, providerLabel, editor)` dans `app-content.js` — s'affiche entre la réponse LLM et l'écriture dans l'éditeur. Overlay + card blanche avec 3 actions : Copier / Ajouter à la suite / Remplacer le champ. Styles `.artis-reform-overlay` / `.artis-reform-preview` dans `app-override.css`.
- **`capturePageText` (v2.2.7)** : masquer `#giles-root`/`#artis-app-canvas` via `visibility:hidden` (pas `display:none`) — `display:none` déclenche le MutationObserver de `app-content.js` sur l'attribut `style` et crée des entrées parasites dans `pendingStyle`.
- **`dit_interval` storage (v2.2.7)** : `DIT_RELOAD_MS` était hardcodé à 60 000 ms — le réglage page options (`dit_interval` en secondes) n'avait aucun effet. Fix : lire `dit_interval` dans `boot()` (même `chrome.storage.local.get`) et l'appliquer avant `startDitMonitor()`. Valeur acceptée : 15–600 s.
- **`swKeepAlive` (v2.2.7)** : appeler `swStopAlive()` AVANT de créer un nouveau `setInterval` — sans ça, deux appels parallèles à `askLLM` créent deux timers keep-alive qui tournent en parallèle sans jamais s'annuler.
- **XSS `meta.model`/`meta.provider` (v2.2.7)** : dans `addBubble()` (`giles.js`), ces valeurs passaient dans `innerHTML` sans `esc()`. Fix : entourer de `esc()` avant interpolation. Risque réel faible (valeurs contrôlées côté SW) mais règle à respecter pour toute future valeur venue d'un provider externe.
- **Import JSON options (v2.2.7)** : valider `giles_fallback_order` (doit être `Array` de providers connus) et `giles_fallback_enabled` (doit être objet, pas array) avant `chrome.storage.local.set`. Un fichier mal formé pouvait injecter de mauvaises valeurs.
- **`manifest.json` host_permissions (v2.2.7)** : `http://localhost/*` retiré (résidu dev, trop large). Ne pas réajouter — si DAI doit tourner en local, utiliser `http://localhost:11434/*` ou laisser l'utilisateur configurer l'URL DAI. `CHANGELOG.md` retiré de `web_accessible_resources` — la page options y accède via `chrome.runtime.getURL` sans en avoir besoin.
- **Model ID Claude (v2.2.7)** : `claude-haiku-4-5-20251001` était un snapshot inexistant → erreur MODEL. Utiliser `claude-haiku-4-5` (sans date) ou vérifier l'ID exact dans la doc Anthropic avant de le coder.
