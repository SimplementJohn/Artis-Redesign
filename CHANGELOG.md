...
- Gilles chat : provider IA affiché en gris au-dessus de chaque réponse bot (`DAI · codellama:7b`, `Gemini · 2.5-flash`…).
- Sidebar : label version `vX.Y.Z` en bas de `.aside-primary`, gris centré, toujours visible (indépendant du bouton GitHub).
- Gilles langue : directive français forcée en tête ET pied du system prompt (`RÈGLE ABSOLUE`) — corrige les réponses en anglais/chinois des modèles locaux faibles.
- DAI modèle par défaut changé `gemma3:4b` → `codellama:7b`.
- Options : badge DAI toujours « configuré » (URL par défaut disponible même sans save explicite).
- Fix `_systemPrompt` : fallback identité Gilles posé avant le prepend lang rule (évite prompt vide si GILLES.md absent).
- Fix `#artis-ver-label` : sorti de `injectVersionButton`, appelé inconditionnellement dans `init()`.
- Fix toolbarObs TinyMCE : analyse des sources de disparition du bandeau Reformuler (doc interne).
- Options : `fetchDAIModels()` appelé automatiquement au chargement si DAI activé.

## 2.2.3 — 2026-06-17

**Defaults DAI + mode limité par défaut :**
- Mode limité = nouveau défaut au premier lancement (au lieu de clair).
- DAI : URL par défaut `http://172.28.1.232:11434`, modèle par défaut `codellama:7b`.
- DAI en 1er dans la chaîne fallback, Gemini en 2e (`options.js` + `giles-bg.js`).
- `chrome.runtime.onInstalled` : écrit tous les défauts DAI + thème à l'installation.
- Input URL DAI pré-rempli avec le défaut si vide.

## 2.2.2 — 2026-06-17

**Nouveau mode « Limité » :**
- Thème clair (page native Artis) + sidebar violette (indigo `#6366f1`) uniquement.
- Pas de canvas animé, pas de recolor, pas de zoom sur les blocs planning.
- Bouton « Limité » en carte cochable sous les 3 modes (Sombre/Auto/Clair) dans popup + options.
- `html.artis-limited` posé par `theme-init.js` (anti-FOUC) et `app-content.js`.
- Bouton toggle sidebar : en mode limité, bascule vers sombre (pas vers clair).
