...
async function getSystemPrompt() {
  if (_systemPrompt === null) {
    ...
    if (!_systemPrompt) _systemPrompt = 'Tu es Gilles, assistant du site Artis.net. Tu aides les utilisateurs à utiliser Artis.';
    /* Directive langue forcée en tête ET en pied — garantit le français même sur modèles locaux faibles */
    const langRule = 'RÈGLE ABSOLUE : Réponds UNIQUEMENT en français. Jamais en anglais, chinois ou autre langue. Toujours en français.';
    _systemPrompt = langRule + '\n\n' + _systemPrompt + '\n\n' + langRule;
  }
...
async function getDAIUrl() {
  ...
  return (s.dai_url || 'http://172.28.1.232:11434').replace(/\/$/, '');
}
...
/* ── Defaults à l'installation ───────────────────────────────── */
chrome.runtime.onInstalled.addListener(({ reason }) => {
  if (reason !== 'install') return;
  chrome.storage.local.get(['artis_theme_mode', 'giles_fallback_order', 'giles_fallback_enabled', 'giles_model_pref_dai', 'dai_url'], s => {
    const defaults = {};
    if (!s.artis_theme_mode)       defaults.artis_theme_mode      = 'limited';
    if (!s.giles_fallback_order)   defaults.giles_fallback_order  = ['dai', 'gemini', 'openai', 'claude'];
    if (!s.giles_fallback_enabled) defaults.giles_fallback_enabled = { dai: true, gemini: true, openai: false, claude: false };
    if (!s.giles_model_pref_dai)   defaults.giles_model_pref_dai  = 'codellama:7b';
    if (!s.dai_url)                defaults.dai_url               = 'http://172.28.1.232:11434';
