...
(function () {
  ...
  const CFG = { themeMode: 'limited', versionBtn: true };  /* themeMode: dark | light | auto | limited — défaut limited (v2.2.2) */
  ...
  function injectVersionLabel() {
    const sidebar = document.querySelector('.aside-primary');
    if (!sidebar || sidebar.querySelector('#artis-ver-label')) return;
    const lbl = document.createElement('div');
    lbl.id = 'artis-ver-label';
    lbl.textContent = 'v' + ARTIS_VERSION;
  ...
  function injectMenuButton(tries) {
    ...
    /* Hover : ouvre/ferme via html.artis-menu-open.
       - Ouverture : debounce 280ms → passage rapide de souris ne déclenche rien.
       - Fermeture : grâce 300ms pour traverser l'espace sidebar → popup. */
    let closeTimer = null;
    let openTimer  = null;

    const open = () => {
      if (closeTimer) { clearTimeout(closeTimer); closeTimer = null; }
      if (document.documentElement.classList.contains('artis-menu-open')) return;
      if (openTimer) return;
    ...
    popup.addEventListener('mouseenter', openImmediate);   /* déjà ouvert → pas de délai */
  ...
  function applyThemePreference() {
    let light = false;
    if (CFG.themeMode === 'light' || CFG.themeMode === 'limited') light = true;
    else if (CFG.themeMode === 'auto') {
      try { light = window.matchMedia('(prefers-color-scheme: light)').matches; } catch (e) {}
    }
    /* add ET remove : corrige une éventuelle supposition de theme-init.js
       (document_start, défaut clair) si le mode résolu est sombre. */
    document.documentElement.classList.toggle('artis-light', light);
    /* limited = thème clair avec sidebar violette uniquement */
    document.documentElement.classList.toggle('artis-limited', CFG.themeMode === 'limited');
  ...
  function init() {
    ...
    injectVersionLabel();       /* label vX.Y.Z en bas sidebar — indépendant du toggle versionBtn */
  ...
  /* Miroir localStorage (sync, lu par theme-init.js au document_start pour
     tuer le FOUC). La vérité reste chrome.storage.local (async). */
  ...
  function boot() {
    chrome.storage.local.get(['artis_enabled', 'artis_theme_mode', 'artis_dark', 'artis_version_btn', 'dit_interval'], s => {
      ...
      if (mode !== 'dark' && mode !== 'light' && mode !== 'auto' && mode !== 'limited') {
        ...
        if (s.artis_dark === false) mode = 'light';
          ...
          mode = (saved === 'dark' || s.artis_dark === true) ? 'dark' : 'limited';
        }
        chrome.storage.local.set({ artis_theme_mode: mode });
      }
      mirror('artis-theme-mode', mode);
      CFG.themeMode  = mode;
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    if ('artis_theme_mode' in changes) {
      ...
      mirror('artis-theme-mode', nv);
      let toLight = nv === 'light' || nv === 'limited';
      if (nv === 'auto') {
      ...
      if (toLight !== document.documentElement.classList.contains('artis-light')) {
        ...
        /* Même côté (ex: light ↔ limited) → reload simple pour appliquer/retirer artis-limited */
        location.reload();
      }
      return;
    }
  ...
  /* boot() toujours appelé : il gère lui-même le cas désactivé
     (disableThemeSheets). L'ancien garde-fou sautait boot() → la feuille
     restait active = « désactivé mais thème toujours là ». */
  boot();
})();
